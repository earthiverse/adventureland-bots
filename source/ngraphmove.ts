...
// Other variables
/** Right now, character.base is set up in add_character() in game.js, where these base coordinates are set. */
const BASE = {
    h: 8,
    v: 7,
    vn: 2
...
// Cost variables
const TRANSPORT_COST = Game.character.speed * TRANSPORT_TIME / 1000
const TOWN_COST = Game.character.speed * TOWN_TIME / 1000
...
export class NGraphMove {
    ...
    public stop(): void {
        this.reset()
        if (Game.character.moving) stop()
        if (Game.character.c.town) stop("town")
    }
    ...
    public canMove(from: NodeData, to: NodeData): boolean {
        ...
        let x = Math.trunc(from.x) - Game.G.geometry[from.map].min_x, y = Math.trunc(from.y) - Game.G.geometry[from.map].min_y
    ...
    private async addToGraph(map: MapName): Promise<unknown> {
        ...
        const mapWidth = Game.G.geometry[map].max_x - Game.G.geometry[map].min_x
        const mapHeight = Game.G.geometry[map].max_y - Game.G.geometry[map].min_y
        ...
        for (const yLine of Game.G.geometry[map].y_lines) {
            for (let y = Math.max(0, yLine[0] - Game.G.geometry[map].min_y - BASE.vn - EXTRA_PADDING); y < yLine[0] - Game.G.geometry[map].min_y + BASE.v + EXTRA_PADDING && y < mapHeight; y++) {
                for (let x = Math.max(0, yLine[1] - Game.G.geometry[map].min_x - BASE.h - EXTRA_PADDING); x < yLine[2] - Game.G.geometry[map].min_x + BASE.h + EXTRA_PADDING && x < mapWidth; x++) {
                    grid[y][x] = UNWALKABLE
                }
            }
        }
        // 2B: Make the x_lines unwalkable
        for (const xLine of Game.G.geometry[map].x_lines) {
            for (let x = Math.max(0, xLine[0] - Game.G.geometry[map].min_x - BASE.h - EXTRA_PADDING); x < xLine[0] - Game.G.geometry[map].min_x + BASE.h + EXTRA_PADDING && x < mapWidth; x++) {
                for (let y = Math.max(0, xLine[1] - Game.G.geometry[map].min_y - BASE.vn - EXTRA_PADDING); y < xLine[2] - Game.G.geometry[map].min_y + BASE.v + EXTRA_PADDING && y < mapHeight; y++) {
                    grid[y][x] = UNWALKABLE
                }
            }
        }
        // 2C: Fill in the grid with walkable pixels
        for (const spawn of Game.G.maps[map].spawns) {
            let x = Math.trunc(spawn[0]) - Game.G.geometry[map].min_x
            let y = Math.trunc(spawn[1]) - Game.G.geometry[map].min_y
        ...
        for (let y = 1; y < mapHeight - 1; y++) {
            for (let x = 1; x < mapWidth - 1; x++) {
                ...
                const nodeID = createNodeId(map, x + Game.G.geometry[map].min_x, y + Game.G.geometry[map].min_y)
                if (this.graph.hasNode(nodeID)) {
                    newNodes.push(this.graph.getNode(nodeID))
                    continue
                }
                const nodeData = createNodeData(map, x + Game.G.geometry[map].min_x, y + Game.G.geometry[map].min_y)
        ...
        for (const npc of Game.G.maps[map].npcs) {
            ...
            for (const map in Game.G.npcs.transporter.places) {
                const spawnID = Game.G.npcs.transporter.places[map as MapName]
                const spawn = Game.G.maps[map as MapName].spawns[spawnID]
        ...
        for (const door of Game.G.maps[map].doors) {
            ...
            const spawn2 = Game.G.maps[door[4]].spawns[door[5]]
        ...
        for (let i = 0; i < newNodes.length; i++) {
            for (let j = i + 1; j < newNodes.length; j++) {
                ...
                // if (can_move({ map: nodeI.data.map, x: nodeI.data.x, y: nodeI.data.y, going_x: nodeJ.data.x, going_y: nodeJ.data.y, base: BASE })) {
        ...
        const townNodeID = createNodeId(map, Game.G.maps[map].spawns[0][0], Game.G.maps[map].spawns[0][1])
    ...
    public async prepare(start: MapName = FIRST_MAP): Promise<void> {
        ...
        for (const destination in Game.G.npcs.transporter.places) {
    ...
    private getPath(goal: NodeData): PathData {
        ...
        this.graph.forEachNode((node: { data: NodeData, id: NodeId }) => {
            ...
            if (node.data.map == Game.character.map) {
                const distance = Math.sqrt((node.data.x - Game.character.x) ** 2 + (node.data.y - Game.character.y) ** 2)
        ...
        if (rawPath[rawPath.length - 1].data.x != Game.character.x || rawPath[rawPath.length - 1].data.y != Game.character.y) {
            // Add the starting position
            optimizedPath.push([NGraphMove.cleanPosition(Game.character), rawPath[rawPath.length - 1].data, undefined])
        }
        ...
        const character = NGraphMove.cleanPosition(Game.character)
        let canMove = 0
    ...
    public async move(goal: PositionReal, finishDistanceTolerance = 0): Promise<unknown> {
        ...
        if (distance(Game.character, goal) <= finishDistanceTolerance) return // We're already close enough

        const from: NodeData = NGraphMove.cleanPosition(Game.character)
        ...
        for (let i = 0; i < path.length;) {
            ...
            let distance = Math.sqrt((Math.trunc(toData.x) - Math.trunc(Game.character.x)) ** 2 + (Math.trunc(toData.y) - Math.trunc(Game.character.y)) ** 2)
            ...
            if (Game.character.c.town || Game.character.moving) {
                await new Promise(resolve => setTimeout(resolve, SLEEP_FOR_MS))
                continue
            }

            if ((!linkData && !can_move_to(toData.x, toData.y)) || Game.character.map !== fromData.map) {
            ...
            if (ENABLE_BLINK && can_use("blink") && Game.character.mp > Game.G.skills.blink.mp) {
                let j = i
                for (; j < path.length; j++) {
