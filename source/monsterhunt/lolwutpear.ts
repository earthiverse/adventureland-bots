...
function prepareMage(bot: AL.Mage) {
    const strategy: Strategy = {
        crab: {
            attack: async () => { await attackTheseTypesMage(bot, ["crab"], information.friends) },
            attackWhileIdle: true,
            equipment: { mainhand: "wand", offhand: "wbook0" },
            move: async () => {
                if (bot.id == information.bot1.name) {
                    await bot.smartMove({ map: "main", x: -1202.5, y: -66 })
                } else if (bot.id == information.bot2.name) {
                    await bot.smartMove({ map: "main", x: -1202.5 - 50, y: -66 - 50 })
...
function prepareMerchant(bot: AL.Merchant) {
    ...
    startMerchant(bot, information, strategy, { map: "main", x: -230, y: -100 })
}
