...
import { bankingPosition, desertlandScorpions } from "../base/locations.js"
...
export async function startMage(bot: Mage, merchant: string, friends: Character[], partyLeader: string, partyMembers: string[]) {
    ...
    async function attackLoop() {
        try {
            ...
            // Idle strategy
            await attackTheseTypesMage(bot, ["scorpion"], friends)
        } catch (e) {
            console.error(e)
        }
    ...
    async function moveLoop() {
        try {
            ...
            await goToNearestWalkableToMonster2(bot, ["scorpion"], desertlandScorpions)
        } catch (e) {
            console.error(e)
        }
