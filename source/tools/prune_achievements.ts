...
AL.Game.loginJSONFile("../../credentials.json").then(async () => {
    ...
    for (const achievement of await AL.AchievementModel.find({}, { _id: 0, name: 1, date: 1 }).sort({ name: 1, date: -1 }).lean().exec()) {
        ...
        if (achievement.date > oneWeekAgo) {
            // Keep hourly
            if (lastDate - achievement.date < ONE_HOUR_IN_MS) {
                ...
                queue.enqueue(async () => {
                    ...
                    await AL.AchievementModel.deleteOne({ date: achievement.date, name: achievement.name }).lean().exec()
                })
            }
    ...
    if (queue.shouldRun) queue.start()
    else AL.Database.disconnect()
})