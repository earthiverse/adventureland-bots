import AL, { type IAchievement } from "alclient"
...
for (const achievement of await AL.AchievementModel.find({}, { _id: 1, name: 1, date: 1 })
    .sort({ name: 1, date: -1 })
    .lean<(Pick<IAchievement, "name" | "date"> & { _id: string })[]>()
    .exec()) {
    ...
    if (achievement.date <= oneYearAgo) {
        ...
        if (lastDate - achievement.date < ONE_WEEK_IN_MS) {
            ...
            console.log(
                `deleting ${achievement.name} @ ${achievement.date} (over 1 year, same week) (${achievement._id})`,
            )
        ...
        if (lastDate - achievement.date < ONE_DAY_IN_MS) {
            // Delete this hourly data
            console.log(
                `deleting ${achievement.name} @ ${achievement.date} (over 1 month, same day) (${achievement._id})`,
            )
            idsToDelete.push(achievement._id)
            continue
        }
        ...
        if (lastDate - achievement.date < ONE_HOUR_IN_MS) {
            // Delete this hourly data
            console.log(
                `deleting ${achievement.name} @ ${achievement.date} (over 1 week, same hour) (${achievement._id})`,
            )
            idsToDelete.push(achievement._id)
            continue
        }
        ...
        if (lastDate - achievement.date < 900000) {
            ...
            console.log(
                `deleting ${achievement.name} @ ${achievement.date} (within 1 week, same 15 minute interval) (${achievement._id})`,
            )
            idsToDelete.push(achievement._id)
            continue
        }
