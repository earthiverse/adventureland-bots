...
import { bankingPosition, mainFrogs } from "../base/locations.js"
...
export async function startMage(bot: Mage, merchant: string, friends: Character[], partyLeader: string, partyMembers: string[]) {
    ...
    async function attackLoop() {
        try {
            ...
            // Idle strategy
            await attackTheseTypesMage(bot, ["frog", "tortoise"], friends)
        } catch (e) {
            console.error(e)
        }
    ...
    async function moveLoop() {
        try {
            ...
            await goToNearestWalkableToMonster2(bot, ["frog", "tortoise"], mainFrogs)
        } catch (e) {
            console.error(e)
        }
        bot.timeouts.set("moveLoop", setTimeout(moveLoop, 250))
