import { CharacterData, EntitiesData, EvalData, GameResponseData, UIData } from "./definitions/adventureland-server"
...
export class Merchant extends PingCompensatedPlayer {
    ...
    */
    public fish(): Promise<void> {
        ...
        const fished = new Promise<void>((resolve, reject) => {
            ...
            const failCheck2 = (data: UIData) => {
                if (data.type == "fishing_fail" && data.name == this.character.id) {
    ...
    public mine(): Promise<void> {
        ...
        if (this.character.c.mining) startedMining = true // We're already mining!?
        const mined = new Promise<void>((resolve, reject) => {
            ...
            const failCheck2 = (data: UIData) => {
                if (data.type == "mining_fail" && data.name == this.character.id) {
    ...
    public massProduction(): Promise<void> {
        const massProductioned = new Promise<void>((resolve, reject) => {
            ...
            setTimeout(() => {
                this.socket.removeListener("ui", productedCheck)
                reject(`massProduction timeout (${TIMEOUT}ms)`)
            }, TIMEOUT)
