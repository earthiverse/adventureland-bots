import AL, { CharacterType, ItemName, Mage, MonsterName, Paladin, PingCompensatedCharacter, Priest, Ranger, Rogue, ServerIdentifier, ServerRegion, Warrior } from "alclient"
...
export async function startBot(characterName: string, characterType: CharacterType, serverRegion: ServerRegion, serverIdentifier: ServerIdentifier, monsters: MonsterName[]) {
    ...
    switch (characterType) {
        ...
        case "paladin":
            try {
                bot = await AL.Game.startPaladin(characterName, serverRegion, serverIdentifier)
            } catch (e) {
                console.error(e)
                setTimeout(startBot, getMsToNextMinute() + BUFFER, characterName, characterType, serverRegion, serverIdentifier, monsters)
                return
        ...
        case "warrior":
            try {
                bot = await AL.Game.startWarrior(characterName, serverRegion, serverIdentifier)
            } catch (e) {
                console.error(e)
                setTimeout(startBot, getMsToNextMinute() + BUFFER, characterName, characterType, serverRegion, serverIdentifier, monsters)
                return
    ...
    switch (characterType) {
        case "mage":
            ...
            break
        case "paladin":
            attackStrategy = new PaladinAttackStrategy({ contexts: CONTEXTS, typeList: monsters })
            break
        case "priest":
            ...
            break
        case "ranger":
            attackStrategy = new RangerAttackStrategy({ contexts: CONTEXTS, typeList: monsters })
            break
        case "rogue":
            ...
            break
        case "warrior":
            attackStrategy = new WarriorAttackStrategy({ contexts: CONTEXTS, enableGreedyAggro: true, typeList: monsters })
            break
    }
...
updateData()
setTimeout(startBot, getMsToNextMinute() + BUFFER, "earthPri2", "priest", "ASIA", "I", ["mechagnome"])
setTimeout(startBot, getMsToNextMinute() + BUFFER, "earthMag2", "mage", "ASIA", "I", ["mechagnome"])
setTimeout(startBot, getMsToNextMinute() + BUFFER, "earthWar2", "warrior", "ASIA", "I", ["mechagnome"])
