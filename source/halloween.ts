    ...
    async function compoundLoop() {
        try {
            ...
            for (const iN in duplicates) {
                ...
                if (ITEMS_TO_SELL[itemName] && itemInfo.level !== undefined && itemInfo.level >= ITEMS_TO_SELL[itemName]) continue // Don't upgrade items we want to sell
    ...
    const serverLoop = async () => {
        try {
            ...
            if (bestMonster && bestMonster.serverRegion !== region || bestMonster.serverIdentifier !== identifier) {
                console.log(`Disconnecting from ${region} ${identifier}`)
