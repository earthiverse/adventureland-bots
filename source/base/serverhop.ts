...
export async function getValentinesMonsterPriority(avoidPVP = false) {
    if (!AL.Database.connection) return []

    const monsterPriority: MonsterName[] = ["pinkgoo"]
    const serverPriority = ["EUI", "EUII", "USI", "USII", "USIII", "ASIAI", "EUPVP", "USPVP"]
...
export async function getEasterMonsterPriority(avoidPVP = false) {
    if (!AL.Database.connection) return []

    const monsterPriority: MonsterName[] = ["wabbit"]
    const serverPriority = ["EUI", "EUII", "USI", "USII", "USIII", "ASIAI", "EUPVP", "USPVP"]
