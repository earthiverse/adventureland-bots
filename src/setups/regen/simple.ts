...
const REGEN_ITEMS: ItemKey[] = ["hpot0", "hpot1", "mpot0", "mpot1"];

function calculateItemScore(item: ItemKey, character: Character): number {
...
function calculateSkillScore(skill: SkillKey, character: Character): number {
  const missingHp = character.max_hp - character.hp;
  const missingMp = character.max_mp - character.mp;
...
function getComparator(character: Character): Comparator<ItemKey | SkillKey> {
  return (a, b) => {
    let aScore = 0;
