...
class NGraphMove {
    ...
    async addToGraph(map) {
        ...
        for (const yLine of G.geometry[map].y_lines) {
            for (let y = Math.min(0, yLine[0] - G.geometry[map].min_y - parent.character.base.v - EXTRA_PADDING); y < yLine[0] - G.geometry[map].min_y + parent.character.base.vn + EXTRA_PADDING && y < mapHeight; y++) {
                for (let x = Math.min(0, yLine[1] - G.geometry[map].min_x - parent.character.base.h - EXTRA_PADDING); x < yLine[2] - G.geometry[map].min_x + parent.character.base.h + EXTRA_PADDING && x < mapWidth; x++) {
                    grid[y][x] = UNWALKABLE;
                }
            }
        }
...
/* harmony import */ var fastpriorityqueue__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(7);
...
/* harmony import */ var _ngraphmove__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(3);
        ...
        try {
            ...
            if (this.movementTarget && this.movementTarget.position) {
                if (!lastMovementTarget
                    ...
                    this.nGraphMove.move(_ngraphmove__WEBPACK_IMPORTED_MODULE_4__[/* NGraphMove */ "a"].cleanPosition(this.movementTarget.position), this.movementTarget.range);
                    setTimeout(() => { this.moveLoop(); }, Math.max(400, parent.character.ping));
                    return;
                }
            ...
            if (visibleHighPriority.length) {
                ...
                this.nGraphMove.move(_ngraphmove__WEBPACK_IMPORTED_MODULE_4__[/* NGraphMove */ "a"].cleanPosition(closest), parent.character.range);
                setTimeout(() => { this.moveLoop(); }, Math.max(400, parent.character.ping));
                return;
            }
            if (visible.length) {
...
const EASY = 30;
const SPECIAL = 500;
class Warrior extends _character__WEBPACK_IMPORTED_MODULE_0__[/* Character */ "a"] {
    constructor() {
        super();
        this.targetPriority = {
            "arcticbee": {
        ...
        this.mainTarget = "scorpion";
        this.itemsToKeep.push("basher", "bataxe", "candycanesword", "carrotsword", "fireblade", "fsword", "wblade", "woodensword", "mshield", "shield", "sshield", "xshield");
    }
    async run() {
        await super.run();
        this.agitateLoop();
    ...
    scareLoop() {
        try {
            const targets = Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getEntities */ "h"])({ isAttackingUs: true, isRIP: false, isMonster: true });
            let wantToScare = false;
            if (targets.length >= 4) {
                wantToScare = true;
            }
                ...
                for (const target of targets) {
                    if (distance(target, parent.character) > target.range)
                        continue;
                    if (Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* calculateDamageRange */ "a"])(target, parent.character)[1] * 6 * target.frequency <= parent.character.hp)
                        continue;
                    wantToScare = true;
                    break;
                }
            }
            if (!Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* isAvailable */ "m"])("scare")
                || !wantToScare) {
                setTimeout(() => { this.scareLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("scare"));
                return;
            }
            if (parent.character.slots.orb.name == "jacko") {
                use_skill("scare");
                ...
                const items = Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* findItems */ "c"])("jacko");
                if (items.length) {
                    const jackoI = items[0].index;
                    equip(jackoI);
                    use_skill("scare");
    ...
    agitateLoop() {
        try {
            if (Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* isAvailable */ "m"])("agitate")) {
                ...
                for (const id in parent.entities) {
                    ...
                    if (e.rip)
                        continue;
                    if (e.target && !parent.party_list.includes(e.target))
                        continue;
                    ...
                    inAgitateCount++;
                    damage += Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* calculateDamageRange */ "a"])(e, parent.character)[1];
                }
        ...
        setTimeout(() => { this.agitateLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("agitate"));
    }
    warcryLoop() {
        if (Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* isAvailable */ "m"])("warcry")) {
            let count = 0;
            for (const member of parent.party_list) {
                const e = parent.entities[member];
                if (!e)
        ...
        setTimeout(() => { this.warcryLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("warcry"));
    }
    stompLoop() {
        const attackingTargets = Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getEntities */ "h"])({ isAttackingUs: true, isRIP: false });
        if (Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* isAvailable */ "m"])("stomp") && attackingTargets.length) {
        ...
        setTimeout(() => { this.stompLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("stomp"));
    }
    cleaveLoop() {
        const wanted = [];
        const unwanted = [];
        for (const e of Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getEntities */ "h"])({ isMonster: true, isRIP: false, isWithinDistance: G.skills.cleave.range })) {
            if (parent.character.attack < Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* calculateDamageRange */ "a"])(parent.character, e)[0]) {
                unwanted.push(e);
                continue;
            }
            if (!this.targetPriority[e.mtype]) {
        ...
        for (const e of unwanted) {
            unwantedDamage += Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* calculateDamageRange */ "a"])(e, parent.character)[1];
        }
        if (Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* isAvailable */ "m"])("cleave") && wanted.length > 3 && unwantedDamage < 1000) {
            use_skill("cleave");
            reduce_cooldown("cleave", Math.min(...parent.pings));
        }
        setTimeout(() => { this.cleaveLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("cleave"));
    }
    chargeLoop() {
        if (Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* isAvailable */ "m"])("charge"))
            use_skill("charge");
        setTimeout(() => { this.chargeLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("charge"));
    }
    hardshellLoop() {
        const targets = Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getEntities */ "h"])({ isAttackingUs: true, isRIP: false });
        if (Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* isAvailable */ "m"])("hardshell")
            && targets.length
            && distance(targets[0], parent.character) <= targets[0].range
            && parent.character.hp < Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* calculateDamageRange */ "a"])(targets[0], parent.character)[1] * 5) {
            use_skill("hardshell");
            reduce_cooldown("hardshell", Math.min(...parent.pings));
        }
        setTimeout(() => { this.hardshellLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("hardshell"));
    }
    async tauntLoop() {
        try {
            let incomingDamage = 0;
            const attackingUs = Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getEntities */ "h"])({ isAttackingUs: true, isRIP: false, isMonster: true });
            for (const e of attackingUs) {
                incomingDamage += Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* calculateDamageRange */ "a"])(e, parent.character)[1];
            }
            if (incomingDamage < 1000 && attackingUs.length < 3) {
                const attackingParty = Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getEntities */ "h"])({ isAttackingParty: true, isAttackingUs: false, isMonster: true, isRIP: false, isWithinDistance: G.skills.taunt.range });
                for (const e of attackingParty) {
                    if (!this.wantToAttack(e, "taunt"))
                        continue;
                    const damage = Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* calculateDamageRange */ "a"])(e, parent.character)[1];
                    if (incomingDamage + damage > 1000)
                        continue;
                    await use_skill("taunt", e);
                    reduce_cooldown("taunt", Math.min(...parent.pings));
                    setTimeout(() => { this.tauntLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("taunt"));
                    return;
                }
                const notAttacking = Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getEntities */ "h"])({ isAttackingParty: false, isMonster: true, isRIP: false, isWithinDistance: G.skills.taunt.range });
                for (const e of notAttacking) {
                    ...
                    if (incomingDamage + damage > 1000)
                        continue;
                    const d = distance(parent.character, e);
                    if (d > parent.character.range && e.range > parent.character.range * 4)
                        continue;
                    await use_skill("taunt", e);
                    reduce_cooldown("taunt", Math.min(...parent.pings));
                    setTimeout(() => { this.tauntLoop(); }, Object(_functions__WEBPACK_IMPORTED_MODULE_2__[/* getCooldownMS */ "e"])("taunt"));
