...
// EXPORTS
__webpack_require__.d(__webpack_exports__, "merchant", function() { return /* binding */ merchant; });

// EXTERNAL MODULE: ./source/character.ts
var character = __webpack_require__(9);

// EXTERNAL MODULE: ./source/functions.ts
var functions = __webpack_require__(0);

// CONCATENATED MODULE: ./source/upgrade.ts

function compoundItem(itemname, targetLevel) {
    if (parent.character.map == "bank")
        return;
    if (!Object(functions["c" /* findItems */])("computer").length) {
    ...
    if (parent.character.q && parent.character.q["compound"])
        return;
    let items;
    for (let itemLevel = 0; itemLevel < targetLevel; itemLevel++) {
        items = Object(functions["d" /* findItemsWithLevel */])(itemname, itemLevel);
        if (items.length >= 3)
            break;
    }
    if (items.length < 3)
    ...
    let scroll;
    if (grade == 0) {
        scroll = Object(functions["b" /* findItem */])("cscroll0");
    }
    else if (grade == 1) {
        scroll = Object(functions["b" /* findItem */])("cscroll1");
    }
    else if (grade == 2) {
        scroll = Object(functions["b" /* findItem */])("cscroll2");
    }
    compound(items[0].index, items[1].index, items[2].index, scroll.index);
}
function upgradeItem(itemname, targetLevel) {
    if (parent.character.map == "bank")
        return;
    ...
    const item = Object(functions["b" /* findItem */])(itemname);
    if (item && item.level < targetLevel) {
        const grade = item_grade(item);
        let scroll;
        if (grade == 0) {
            scroll = Object(functions["b" /* findItem */])("scroll0");
        }
        else if (grade == 1) {
            scroll = Object(functions["b" /* findItem */])("scroll1");
        }
        else if (grade == 2) {
            scroll = Object(functions["b" /* findItem */])("scroll2");
        }
        if (scroll)
            upgrade(item.index, scroll.index);
        return;
    }
}
function compoundIfMany(maxLevel) {
    if (parent.character.map == "bank")
        return;
    ...
    const items = [];
    for (const item of Object(functions["i" /* getInventory */])()) {
    ...
    for (let i = 0; i < items.length - 2; i++) {
        if (items[i].name == items[i + 1].name && items[i].name == items[i + 2].name
            && items[i].level == items[i + 1].level && items[i].level == items[i + 2].level) {
...
function upgradeIfMany(maxLevel) {
    if (parent.character.map == "bank")
        return;
    if (parent.character.q.upgrade)
        return;
    ...
    for (let i = 0; i < 42; i++) {
        if (!parent.character.items[i])
            continue;
        if (!(G.items[parent.character.items[i].name].upgrade))
            continue;
        const items = Object(functions["c" /* findItems */])(parent.character.items[i].name);
        if (items.length == 1)
            continue;
        items.sort((a, b) => {
            return a.level - b.level;
        ...
        let scrolls;
        if (grade == 0) {
            scrolls = Object(functions["b" /* findItem */])("scroll0");
        }
        else if (grade == 1) {
            scrolls = Object(functions["b" /* findItem */])("scroll1");
        }
        else if (grade == 2) {
            scrolls = Object(functions["b" /* findItem */])("scroll2");
        }
        if (scrolls)
            upgrade(item.index, scrolls.index);
        return;
    }
}
...
// EXTERNAL MODULE: ./source/ngraphmove.ts
var ngraphmove = __webpack_require__(3);

// CONCATENATED MODULE: ./source/merchant.ts
...
class merchant_Merchant extends character["a" /* Character */] {
    constructor() {
        super();
        this.targetPriority = {
            "bee": {
        ...
        this.didBankStuff = 0;
        this.luckedCharacters = {};
        this.itemsToKeep.splice(this.itemsToKeep.indexOf("lantern"), 1);
        this.itemsToKeep.splice(this.itemsToKeep.indexOf("jacko"), 1);
        this.itemsToKeep.push("cscroll0", "cscroll1", "cscroll2", "scroll0", "scroll1", "scroll2", "stand0", "dexscroll", "intscroll", "strscroll", "monstertoken", "platinumnugget", "bow");
    }
    getMovementTarget() {
        if (parent.character.rip) {
            set_message("RIP");
        ...
        for (const mtype in parent.S) {
            ...
            if (!info.target)
                continue;
            set_message(mtype);
            return { target: mtype, position: info, range: 50 };
        }
        const party = Object(info["c" /* getPartyInfo */])();
        for (const name in party) {
            if (name == parent.character.name)
                continue;
        ...
        const players = Object(info["d" /* getPlayersInfo */])();
        for (const name in players) {
            ...
            if (distance(parent.character, player) <= G.skills.mluck.range && (!player.s.mluck || Date.now() - players[name].lastSeen.getTime() > 3000000 || player.s.mluck.ms < 1800000)) {
                delete players[name];
                Object(info["h" /* setPlayersInfo */])(players);
                break;
            }
        ...
        const npcs = Object(info["b" /* getNPCInfo */])();
    ...
    async run() {
        try {
            ...
            const before = Date.now();
            this.nGraphMove = await ngraphmove["a" /* NGraphMove */].getInstance();
            game_log(`Took ${Date.now() - before}ms to prepare pathfinding.`);
            this.nGraphMove.getGraphInfo();
        }
        catch (e) {
    ...
    async newBankStuff() {
        ...
        let emptyBankSlots = Object(functions["f" /* getEmptyBankSlots */])();
        for (const item of Object(functions["i" /* getInventory */])()) {
            if (emptyBankSlots.length == 0)
                break;
            if (this.itemsToKeep.includes(item.name))
                continue;
        ...
        function canCombine(a, b, c, d) {
            if (a.name != b.name || a.name != c.name || a.name != d.name)
                return false;
            if (a.p != b.p || a.p != c.p || a.p != d.p)
                return false;
            if (b.level != c.level || b.level != d.level)
                return false;
            if (!G.items[a.name].compound)
                return false;
            if (b.level >= Number.parseInt("4"))
                return false;
            return true;
        }
        function canUpgrade(a, b) {
            if (a.name != b.name)
                return false;
            if (a.p != b.p)
                return false;
            if (!G.items[a.name].upgrade)
                return false;
            if (b.level >= Number.parseInt("8"))
                return false;
            return true;
        }
        function isSameStackable(a, b) {
            if (a.name != b.name)
                return false;
            if (!a.q)
                return false;
        ...
        const wantToSell = (a) => {
            if (!this.itemsToSell[a.name])
                return false;
            if (a.level && a.level > this.itemsToSell[a.name])
                return false;
            if (a.p)
                return false;
            return true;
        ...
        await Object(functions["p" /* sleep */])(Math.max(...parent.pings));
        let emptySlots = Object(functions["g" /* getEmptySlots */])(parent.character.items);
        ...
        await Object(functions["p" /* sleep */])(Math.max(...parent.pings));
        emptySlots = Object(functions["g" /* getEmptySlots */])(parent.character.items);
        ...
        for (let i = 0; i < allItems.length; i++) {
            ...
            if (!this.itemsToExchange.has(item.name))
                continue;
            if (item.q < G.items[item.name].e)
                continue;
            parent.socket.emit("bank", {
                operation: "swap",
                inv: emptySlots.shift(),
        ...
        await Object(functions["p" /* sleep */])(Math.max(...parent.pings));
        emptyBankSlots = Object(functions["f" /* getEmptyBankSlots */])();
        for (const item of Object(functions["i" /* getInventory */])()) {
            if (item.name != "bow")
                continue;
            if (item.level < 9) {
    ...
    async luckLoop() {
        try {
            if (parent.character.mp < 10) {
                setTimeout(() => { this.luckLoop(); }, Object(functions["e" /* getCooldownMS */])("mluck"));
                return;
            }
            if (!parent.character.s.mluck || parent.character.s["mluck"].ms < 10000 || parent.character.s["mluck"].f != parent.character.name) {
                ...
                await Object(functions["p" /* sleep */])(G.skills["mluck"].cooldown);
            }
            for (const id in parent.entities) {
                const luckTarget = parent.entities[id];
                if (!Object(functions["o" /* isPlayer */])(luckTarget)
                    || distance(parent.character, luckTarget) > G.skills.mluck.range
                    || !Object(functions["m" /* isAvailable */])("mluck"))
                    continue;
                if (this.luckedCharacters[luckTarget.name] && this.luckedCharacters[luckTarget.name] > Date.now() - parent.character.ping * 2)
                    continue;
                if (!luckTarget.s || !luckTarget.s["mluck"] || luckTarget.s["mluck"].ms < 3540000 || luckTarget.s["mluck"].f != parent.character.name) {
...
const merchant = new merchant_Merchant();
