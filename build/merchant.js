...
// EXTERNAL MODULE: ./source/character.ts + 2 modules
var character = __webpack_require__(10);
...
function compoundItem(itemname, targetLevel) {
    if (parent.character.map == "bank")
        return;
    if (!Object(functions["d" /* findItems */])("computer").length) {
    ...
    for (let itemLevel = 0; itemLevel < targetLevel; itemLevel++) {
        items = Object(functions["e" /* findItemsWithLevel */])(itemname, itemLevel);
        if (items.length >= 3)
            break;
    }
    if (items.length < 3)
    ...
    let scroll;
    if (grade == 0) {
        scroll = Object(functions["c" /* findItem */])("cscroll0");
    }
    else if (grade == 1) {
        scroll = Object(functions["c" /* findItem */])("cscroll1");
    }
    else if (grade == 2) {
        scroll = Object(functions["c" /* findItem */])("cscroll2");
    }
...
function upgradeItem(itemname, targetLevel) {
    ...
    const item = Object(functions["c" /* findItem */])(itemname);
    if (item && item.level < targetLevel) {
        const grade = item_grade(item);
        let scroll;
        if (grade == 0) {
            scroll = Object(functions["c" /* findItem */])("scroll0");
        }
        else if (grade == 1) {
            scroll = Object(functions["c" /* findItem */])("scroll1");
        }
        else if (grade == 2) {
            scroll = Object(functions["c" /* findItem */])("scroll2");
        }
        if (scroll)
...
function compoundIfMany(maxLevel) {
    ...
    const items = [];
    for (const item of Object(functions["j" /* getInventory */])()) {
...
function upgradeIfMany(maxLevel) {
    ...
    for (let i = 0; i < 42; i++) {
        ...
        if (!(G.items[parent.character.items[i].name].upgrade))
            continue;
        const items = Object(functions["d" /* findItems */])(parent.character.items[i].name);
        if (items.length == 1)
            continue;
        items.sort((a, b) => {
            return a.level - b.level;
        ...
        let scrolls;
        if (grade == 0) {
            scrolls = Object(functions["c" /* findItem */])("scroll0");
        }
        else if (grade == 1) {
            scrolls = Object(functions["c" /* findItem */])("scroll1");
        }
        else if (grade == 2) {
            scrolls = Object(functions["c" /* findItem */])("scroll2");
        }
        if (scrolls)
...
class merchant_Merchant extends character["a" /* Character */] {
    ...
    async mainLoop() {
        try {
            ...
            upgradeIfMany(8);
            compoundIfMany(4);
            await Object(functions["a" /* buyIfNone */])("bow", 9, 4);
            upgradeItem("bow", 9);
    ...
    async newBankStuff() {
        ...
        let emptyBankSlots = Object(functions["g" /* getEmptyBankSlots */])();
        for (const item of Object(functions["j" /* getInventory */])()) {
        ...
        await Object(functions["q" /* sleep */])(Math.max(...parent.pings));
        const allItems = [];
        for (const item of Object(functions["j" /* getInventory */])()) {
            if (this.itemsToKeep.includes(item.name))
                continue;
        ...
        for (const pack in parent.character.bank) {
            if (pack == "gold")
                continue;
            for (const item of Object(functions["j" /* getInventory */])(parent.character.bank[pack])) {
        ...
        await Object(functions["q" /* sleep */])(Math.max(...parent.pings));
        let emptySlots = Object(functions["h" /* getEmptySlots */])(parent.character.items);
        ...
        await Object(functions["q" /* sleep */])(Math.max(...parent.pings));
        emptySlots = Object(functions["h" /* getEmptySlots */])(parent.character.items);
        ...
        await Object(functions["q" /* sleep */])(Math.max(...parent.pings));
        emptyBankSlots = Object(functions["g" /* getEmptyBankSlots */])();
        for (const item of Object(functions["j" /* getInventory */])()) {
            if (item.name != "bow")
                continue;
            if (item.level < 9) {
    ...
    async luckLoop() {
        try {
            if (parent.character.mp < 10) {
                setTimeout(() => { this.luckLoop(); }, Object(functions["f" /* getCooldownMS */])("mluck"));
                return;
            }
            if (!parent.character.s.mluck || parent.character.s["mluck"].ms < 10000 || parent.character.s["mluck"].f != parent.character.name) {
                ...
                await Object(functions["q" /* sleep */])(G.skills["mluck"].cooldown);
            }
            for (const id in parent.entities) {
                const luckTarget = parent.entities[id];
                if (!Object(functions["p" /* isPlayer */])(luckTarget)
                    || distance(parent.character, luckTarget) > G.skills.mluck.range
                    || !Object(functions["n" /* isAvailable */])("mluck"))
                    continue;
